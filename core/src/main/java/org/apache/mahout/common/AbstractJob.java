...
public abstract class AbstractJob extends Configured implements Tool {
	...
	private Map<String, String> argMapBackPort;
	 ...
	 * 
	 */
	public Map<String, String> parseArgumentsBackPort(String[] args)
			throws IOException {
		...
		try {
			parseDirectoriesBackPort(cmdLine);
		...
		argMapBackPort = new TreeMap<String, String>();
		maybePutBackPort(argMapBackPort, cmdLine,
				this.options.toArray(new Option[this.options.size()]));

		this.tempPath = new Path(argMapBackPort.get("--tempDir"));

		log.info("Command line arguments: {}", argMap);
		return argMapBackPort;
	}
	 ...
	 */
	protected void parseDirectoriesBackPort(CommandLine cmdLine) {
	 ...
	 * called, this method will throw an {@code OptionException} if no source
	 * (command-line or property) for that value is present. Otherwise,
	 * {@code inputPath} or {@code outputPath} will be non-null only if
	 * specified as a hadoop property. Command-line options take precedence over
	 * hadoop properties.
	 * 
	 * @param cmdLine
	 * @throws IllegalArgumentException
	...
	protected static void maybePutBackPort(Map<String, String> args, CommandLine cmdLine, Option... opt) {
		    for (Option o : opt) {
